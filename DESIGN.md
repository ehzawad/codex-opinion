...
Reconciliation protocol (when to call Codex, how to frame the context, when to audit the reconciled draft, when to run a closing revision check) lives in [`SKILL.md`](plugins/codex-opinion/skills/codex-opinion/SKILL.md) and Claude's judgment at runtime. [`ask_codex.py`](plugins/codex-opinion/skills/codex-opinion/scripts/ask_codex.py) remains a transport shim: it bookends stdin with a generic, overridable review directive, calls Codex, returns the reply, and saves per-project session state atomically. Pass `--no-default-instruction` for exact stdin passthrough. When `CODEX_OPINION_SESSION_KEY` is set in the environment, the state key includes a hash of that value, giving that caller a separate Codex thread for the same project.

Multi-round behavior — initial round, audit call, closing revision check — is produced by Claude invoking the same script multiple times on the resumed Codex thread with explicit per-call contexts. The script parses Codex's JSONL output only for what transport needs — `thread.started` to capture the session id, `item.completed` → `agent_message.text` for the final reply — and does not count rounds, detect cycle boundaries, or interpret Codex's reply semantically for protocol decisions. Adding protocol state to the script would mix transport with judgment and create edge cases around idempotency, concurrent invocations, and what counts as "stable."
